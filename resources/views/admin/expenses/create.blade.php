    ...
    <form action="{{ route('admin.expenses.store') }}" method="POST" enctype="multipart/form-data" class="space-y-6">
                                ...
                                @foreach($expenseCategories->groupBy('category.name') as $parentName => $items)
                                    <optgroup label="{{ $parentName }}" class="bg-slate-800 text-slate-300 font-bold">
                                        @foreach($items as $item)
                                            <option value="{{ $item->id }}" class="bg-slate-800 text-white font-normal">
                                                {{ $item->type }}
                                            </option>
                                        @endforeach
                                    </optgroup>
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                ...
                <div class="relative group p-4 bg-slate-800/50 rounded-xl border border-slate-700/50 hover:border-emerald-500/50 transition-all duration-300">
                    <div class="flex items-center justify-between mb-2">
                        ...
                        <label class="relative inline-flex items-center cursor-pointer">
                            <input type="checkbox" name="is_tax_deductible" class="sr-only peer">
                ...
                <div class="relative group p-4 bg-slate-800/50 rounded-xl border border-slate-700/50 hover:border-sky-500/50 transition-all duration-300">
                    <div class="flex items-center justify-between mb-2">
                        ...
                        <label class="relative inline-flex items-center cursor-pointer">
                            <input type="checkbox" name="is_billable" class="sr-only peer">
        ...
        <div class="flex flex-col sm:flex-row gap-4 justify-end pt-6 border-t border-slate-700/50">
            ...
            <button type="submit" name="action" value="draft" 
                    class="group relative px-6 py-3 rounded-xl bg-slate-800 border border-indigo-500/30 text-indigo-300 font-medium hover:bg-indigo-500/10 hover:text-indigo-200 hover:border-indigo-500/50 transition-all duration-300 shadow-lg shadow-indigo-900/20 overflow-hidden">
                <div class="absolute inset-0 translate-x-[-100%] group-hover:translate-x-[100%] transition-transform duration-1000 bg-gradient-to-r from-transparent via-indigo-500/10 to-transparent"></div>
